...
export default function CharacterModel(props: any) {
  ...
  const rightHandColliderRef = useRef<THREE.Group | null>(null);
  const leftHandRef = useRef<THREE.Group | null>(null);
  const leftHandColliderRef = useRef<THREE.Group | null>(null);
  ...
  let mugModel: THREE.Bone | null = null;
  ...
  const [punchEffectProps, setPunchEffectProp] = useState({
    ...
    play: false,
    position: [-0.2, -0.2, 0.5] as [number, number, number],
    startFrame: 0,
  });
  ...
  useFrame(() => {
    ...
    if (curAnimation === animationSet.action4) {
      ...
      if (rightHandColliderRef.current) {
        ...
        if (rightHandColliderRef.current && rightHandRef.current) {
  ...
  useEffect(() => {
    // Play animation
    const action = actions[curAnimation ? curAnimation : animationSet.jumpIdle] as THREE.AnimationAction | null;
    ...
    if (
      ...
      if (curAnimation === animationSet.action3) {
        if (mugModel) mugModel.visible = true;
      } else {
    ...
    return () => {
      ...
      if (curAnimation === animationSet.action4) {
        if (rightHandColliderRef.current) {
          (rightHandColliderRef.current as any).setTranslationWrtParent(vec3({ x: 0, y: 0, z: 0 })) as THREE.Vector3
        }
      }
    };
  }, [curAnimation]);
